      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-160 table-fixed border-collapse text-left text-sm">
          <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
            <tr>
              <th className="w-14 px-4 py-4 font-semibold" scope="col"><TrendingUpDown /></th>
              <th className="px-4 py-4 font-semibold" scope="col">Catégorie</th>
              <th className="px-4 py-4 font-semibold" scope="col">Montant</th>
              <th className="px-4 py-4 font-semibold" scope="col">Type</th>
              <th className="px-4 py-4 font-semibold" scope="col">Fréquence</th>
              <th className="px-4 py-4 font-semibold" scope="col">Date</th>
              <th className="w-18 px-4 py-4 text-center font-semibold" scope="col"></th>
            </tr>
          </thead>
            ...
            {transactions && transactions.length > 0 ?
              transactions?.map((transaction) => (
                <tr key={transaction.id} className="transition hover:bg-indigo-50/40">
                  ...
                  <td className={`px-4 py-4 font-semibold ${transaction.type === 'Entrée' ? 'text-emerald-600' : 'text-red-600'}`}>{transaction.type === 'Entrée' ? '+' : '-'}{transaction.amount} €</td>
